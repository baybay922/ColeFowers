...
define(['jquery'],function($) {
	return {
		...
		"inpputFocus2":function(el){
			var placeholder =el.attr("placeholder"),
					regTip = el.next();
			// $(".l_regTip").hide();
			el.siblings(".l_del").show();
			el.attr("data-placeholder",placeholder).css('border','solid 1px #b0d3fb').attr("placeholder","");
			regTip.text(regTip.attr("data-regTip")).removeClass("pjWrong").addClass("pjTip").slideDown();
			el.on("blur",function () {
					el.attr("placeholder",placeholder);
			})
		...
		"scrollLoading":function(el,box) {//第一个参数是内部大盒子，第二个参数一般为body
			$(".l_lastTip").show();
